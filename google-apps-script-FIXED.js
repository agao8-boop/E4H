// Google Apps Script for E4H Form Submission - FIXED VERSION
...
function doPost(e) {
  try {
    ...
    // Define regular form fields in the exact order they should appear
    const textFields = [
      'name',
      'email',
      'phone',
    ...
    // Define single checkbox fields (these send "on" when checked, nothing when unchecked)
    const singleCheckboxFields = ['updates', 'tips'];

    // Define multi-checkbox fields - these can have multiple values
    ...
    // Add regular text form fields - use empty string if blank
    ...
    // These send "on" when checked, nothing when unchecked
